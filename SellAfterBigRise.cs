...
namespace NinjaTrader.Strategy
{
    ...
    /// Sell after big Rise
    /// </summary>
    [Description("Sell after big Rise")]
    public class SellAfterBigRise : Strategy
    {
        #region Variables
        // Wizard generated variables
        private double riseSize = 6.00; // Default setting for RiseSize
        private int riseTime = 16; // Default setting for RiseTime
        private int unitsToSell = 1; // Default setting for UnitsToSell
        private double target = 0.01; // Default setting for Target
        private double stop = 0.002; // Default setting for Stop
		private int barsToWait = 5; // Number of bars to wait after Rise
		private int mfiLength = 17; // MFI parameter
        ...
        protected override void OnBarUpdate()
        {
            ...
            if ((High[BarsToWait] > Low[riseTime+BarsToWait] + riseSize) 
				...
				&& alreadyTradedToday < 2 
				&& (CrossBelow(MFI(MFILength), 75, BarsToWait)))
            {
				alreadyTradedToday += 1;
                entryOrder = EnterShort(UnitsToSell, "");
            }
        }
		...
		protected override void OnExecution(IExecution execution)
		{
			if (entryOrder != null && entryOrder == execution.Order)
				SendMail("naren.salem+auto@gmail.com", "naren.salem@gmail.com", "SellOnBigRise", execution.ToString());
		}
