...
namespace NinjaTrader.Strategy
{
    ...
    /// Buy after big drop
    /// </summary>
    [Description("Buy after big drop With Small Target")]
    public class BuyAfterBigDropWithSmallTarget : Strategy
    {
        #region Variables
        // Wizard generated variables
        private double dropSize = 7.00; // Default setting for DropSize
        private int dropTime = 8; // Default setting for DropTime
        private int unitsToBuy = 1; // Default setting for UnitsToBuy
        private double target = 0.003; // Default setting for Target
        private double stop = 0.02; // Default setting for Stop
		private int barsToWait = 1; // Number of bars to wait after drop
        ...
        protected override void OnBarUpdate()
        {
            ...
            {
				alreadyTradedToday += 1;
                entryOrder = EnterLong(UnitsToBuy, "BigDropSmallTargets");
            }
			
			if (Position.MarketPosition == MarketPosition.Long && BarsSinceEntry() > 300)
			{
				ExitLong();
			}
        }
		...
		protected override void OnExecution(IExecution execution)
		{
			if (entryOrder != null && entryOrder == execution.Order)
				SendMail("naren.salem+auto@gmail.com", "naren.salem@gmail.com", "BigDropSmallTargets", execution.ToString());
		}
